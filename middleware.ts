...
const RATE_LIMIT_MAX = process.env.RATE_LIMIT_MAX !== undefined
  ? parseInt(process.env.RATE_LIMIT_MAX, 10)
  : 50;

const RATE_LIMIT_WINDOW = process.env.RATE_LIMIT_WINDOW !== undefined
  ? parseInt(process.env.RATE_LIMIT_WINDOW, 10) * 1000
  : 60 * 1000;

const MAX_STORE_SIZE = process.env.RATE_LIMIT_MAX_STORE_SIZE !== undefined
  ? parseInt(process.env.RATE_LIMIT_MAX_STORE_SIZE, 10)
  : 10000;

let lastCleanupTime = Date.now();

const CLEANUP_INTERVAL = process.env.RATE_LIMIT_CLEANUP_INTERVAL !== undefined
  ? parseInt(process.env.RATE_LIMIT_CLEANUP_INTERVAL, 10) * 1000
...
export function middleware(request: NextRequest) {
  // Only apply rate limiting for /player routes.
